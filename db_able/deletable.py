...
@ABCRestrictions.require(‘del_params’)
class Deletable(Database):
    ...
    def __compile__(cls):
        ...
        cls._validate_params(‘del_params’)
        cls.delete_params = cls.del_params
    ...
    def delete(self):
        ...
        :rtype: bool
        “””
        stored_procedure = ‘%s_delete%s’ % (cls.__name__, cls.del_params.version)
        validated_args = self.kwargs_validator(*self.del_params, **self)
        with DBClient(cls.db, stored_procedure, *validated_args) as conn:
            assert conn.data, ‘Expected a truthy response for `%s`.`%s`’ % (cls.db, stored_procedure)
